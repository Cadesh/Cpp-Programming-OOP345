	...
	for (int i = 0; i < dSize; i++) {
		for (int j = 0; j < pSize; j++) {
			if (desc[i].code == price[j].code) {
				...
				try {
					//Product* tt = new Product(desc[i].desc, price[j].price);
					unique_ptr<Product> tt(new Product(desc[i].desc, price[j].price));
					tt->validade();
					priceList += std::move(tt);
				}
				catch (...) { //catch all
					throw;
				}
